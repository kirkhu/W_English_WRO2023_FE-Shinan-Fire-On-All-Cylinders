...
| Without the wide-angle lens(未加廣角鏡) |With the wide-angle lens 巳加廣角鏡 |
| :---: | :---: |
...
- In the Raspberry Pi program, it is possible to set the resolution of the camera module. We experimented with the following common resolutions and found that
- 在raspberry pi的程式中可以設定鏡頭模組的解析度，我們曾實驗以下常見的解析度，發現
  ...
  3. 320x240 幀率90p
- 
- 在我們的實驗中，當相機模組的解析度設定為1080x640時，由於高解析度的影像處理需求，程式需要花費大量的時間來辨識積木，這導致了程式的運算效率降低。另一方面，當解析度設定為320x240時，雖然運算效率極高，但由於解析度過低，導致無法正常辨識積木。然而，當解析度設定為640x320時，我們觀察到可以正常辨識積木，而且運算效率也不會太慢，避免了車輛撞上積木的問題。因此，我們最終選擇將相機模組的解析度設定為640x320。
- In our experiments, when the camera module's resolution was set to 1080x640, the high-resolution image processing demands resulted in the program spending a considerable amount of time on block recognition, leading to a decrease in computational efficiency. On the other hand, when the resolution was set to 320x240, the computational efficiency was very high, but the low resolution caused difficulties in accurately recognizing the blocks. However, when we set the resolution to 640x320, we observed that the program could successfully recognize the blocks without compromising computational efficiency, thus avoiding collisions with the blocks. Therefore, we ultimately decided to set the camera module's resolution to 640x320.
